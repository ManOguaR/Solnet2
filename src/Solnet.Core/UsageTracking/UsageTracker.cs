...
using Solnet.Core.Configuration;
using Solnet.Core.Metadata;
using Solnet.Core.UsageTelemetry;
using Solnet.Core.Logging;
...
{
    internal class UsageTracker : IUsageTracker, IAsyncDisposable
    {
        ...
        public UsageTracker(IOptions<UsageTelemetryOptions> usageTrackingOptions)
        {
            ...
            if (_options.Enabled)
            {
                Telemetry = new SolnetCoreUsageTelemetry
                {
                    Id = NewId.NextGuid(),
                    ...
                    {
                        SolnetCoreVersion = HostMetadataCache.Host.SolnetCoreVersion,
        ...
        SolnetCoreUsageTelemetry? Telemetry { get; }
            ...
            where T : ISolnetFactoryConfigurator
        {
            ...
            lock (_lock)
            {
                ...
                var busUsageTelemetry = new SolnetUsageTelemetry
                {
                    Name = busName,
                    Created = DateTimeOffset.Now.ToString("O"),
                    Endpoints = []
                ...
                Telemetry.Solnet ??= [];
                Telemetry.Solnet.Add(busUsageTelemetry);

                var observer = new UsageTelemetrySolnetObserver(this, busUsageTelemetry);
                configurator.ConnectSolnetObserver(observer);
        ...
        public void PostCreateSolnet(ISolnet bus, SolnetUsageTelemetry busTelemetry)
        {
            if (!_options.Enabled)
                return;
        ...
        public void PostStartSolnet(ISolnet bus, SolnetUsageTelemetry busTelemetry)
        {
            if (!_options.Enabled || Telemetry is null)
                return;
            ...
            lock (_lock)
            {
                ...
                if (Telemetry.Solnet?.Any(x => x.Started is null) ?? true)
                    return;
        ...
        async Task ReportUsageTelemetry()
        {
            ...
            try
            {
                ...
                lock (_lock)
                {
                    ...
                    json = JsonSerializer.Serialize(Telemetry!, UsageTelemetrySerializerContext.Default.SolnetCoreUsageTelemetry);
                }
